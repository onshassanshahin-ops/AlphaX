...
import { ArrowLeft, Plus, Users, Copy, Check, Search, UserCheck, UserX, RefreshCw } from 'lucide-react';
...
interface Alphanaut {
  ...
  is_public: boolean;
  created_at: string;
  alphanaut_blocks?: { blocks?: { slug: string; name: string; color: string; icon: string } }[];
}
...
export default function AdminAlphanauts() {
  ...
  const [allBlocks, setAllBlocks] = useState<{ id: string; slug: string; name: string }[]>([]);
                  ...
                  {BLOCKS.map(b => {
                    ...
                    return (
                      <button
                        ...
                        type="button"
                        onClick={() => setForm(f => {
                          if (f.navigatorBlocks.includes(b.id)) {
                            return { ...f, navigatorBlocks: f.navigatorBlocks.filter(x => x !== b.id) };
                          } else if (f.memberBlocks.includes(b.id)) {
                            return { ...f, memberBlocks: f.memberBlocks.filter(x => x !== b.id), navigatorBlocks: [...f.navigatorBlocks, b.id] };
                          } else {
                            return { ...f, memberBlocks: [...f.memberBlocks, b.id] };
                          }
                        })}
